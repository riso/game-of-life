...
describe('Game of life engine', () => {
	...
	describe("world rules", () => {
		...
		it("should determine next generation value according to number of alive neighbors", () => {
			...
			// 0 0 0
			// 0 0 0 -> with this everything is still dead
			// 0 1 1
			game.set(1, 2);
			game.set(2, 2);
			expect(game._nextGen(1, 1)).toBe(false);
			...
			// 0 0 0
			// 0 1 0 -> all three cells should stay alive, and (2, 1) becomes alive
			// 0 1 1
			game.set(1, 1);
			...
			// 0 0 1
			// 0 1 1 -> here (1, 1) and (2, 1) die, everything else should stay alive
			// 0 1 1
			game.set(2, 0);
			game.set(2, 1);
			expect(game._nextGen(1, 1)).toBe(false);
